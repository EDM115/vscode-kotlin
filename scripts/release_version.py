...
def main():
    ...
    pkg_path = str(PROJECT_DIR / "package.json")
    pkg = json.load(open(pkg_path)) 
    version = parse_version(pkg[PROJECT_VERSION_KEY])
    ...
    pkg[PROJECT_VERSION_KEY] = str(new_version)
    with open(pkg_path, "w") as pkg_file:
        json.dump(pkg, pkg_file, indent=4)
